...
void LX8266DMX::setMaxSlots (int slots) {
	_slots = constrain(slots, DMX_MIN_SLOTS, DMX_MAX_SLOTS);
}
...
bool LX8266DMX::setFrame(const uint8_t* data, uint16_t slots) {
	if (data == NULL || slots < DMX_MIN_SLOTS || slots > DMX_MAX_SLOTS) {
		return false;
	}

	noInterrupts();
	_dmxData[0] = 0;
	memcpy(&_dmxData[1], data, slots);
	_slots = slots;
	interrupts();
...
uint16_t LX8266DMX::copyFrame(uint8_t* destination, uint16_t capacity) {
	...
	const uint16_t slots =
		currentSlots < capacity ? currentSlots : capacity;
	memcpy(destination, &_dmxData[1], slots);
	interrupts();
